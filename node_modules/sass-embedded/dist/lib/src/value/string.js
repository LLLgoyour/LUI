...
/** A SassScript string. */
class SassString extends value_1.Value {
    ...
    sassIndexToStringIndex(sassIndex, name) {
        ...
        if (sassIdx === 0) {
            throw utils_1.valueError('String index may not be 0', name);
        }
        const sassLength = this.sassLength();
        if (Math.abs(sassIdx) > sassLength) {
            throw utils_1.valueError(`Invalid index ${sassIdx} for a string with ${sassLength} characters`, name);
        }
        if (sassIdx < 0)
    ...
    hashCode() {
        return immutable_1.hash(this.text);
    }
    toString() {
