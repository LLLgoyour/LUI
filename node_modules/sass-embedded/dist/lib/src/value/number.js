...
/** A SassScript number. */
class SassNumber extends value_1.Value {
    /** Creates a number, optionally with a single numerator unit. */
    constructor(value, unit) {
        super();
        this.valueInternal = value;
        this.numeratorUnitsInternal = unit === undefined ? immutable_1.List([]) : immutable_1.List([unit]);
        this.denominatorUnitsInternal = immutable_1.List([]);
    }
    /**
     ...
     */
    static withUnits(value, options) {
        var _a, _b;
        let numerators = utils_1.asImmutableList((_a = options === null || options === void 0 ? void 0 : options.numeratorUnits) !== null && _a !== void 0 ? _a : []);
        const unsimplifiedDenominators = (_b = options === null || options === void 0 ? void 0 : options.denominatorUnits) !== null && _b !== void 0 ? _b : [];
        const denominators = [];
        ...
        number.denominatorUnitsInternal = immutable_1.List(denominators);
        return number;
    }
    /** `this`'s value. */
    get value() {
    ...
    get isInt() {
        return utils_2.fuzzyIsInt(this.value);
    }
    /**
     ...
     */
    get asInt() {
        return utils_2.fuzzyAsInt(this.value);
    }
     ...
     */
    assertInt(name) {
        const int = utils_2.fuzzyAsInt(this.value);
        if (int === null) {
            throw utils_1.valueError(`${this} is not an int`, name);
        }
        return int;
    }
    /**
    ...
    assertInRange(min, max, name) {
        const clamped = utils_2.fuzzyInRange(this.value, min, max);
        if (clamped === null) {
            throw utils_1.valueError(`${this} must be between ${min} and ${max}`, name);
        }
        return clamped;
    }
    /**
    ...
    assertNoUnits(name) {
        if (this.hasUnits) {
            throw utils_1.valueError(`Expected ${this} to have no units`, name);
        }
        return this;
    }
    /**
    ...
    assertUnit(unit, name) {
        if (!this.hasUnit(unit)) {
            throw utils_1.valueError(`Expected ${this} to have no unit ${unit}`, name);
        }
        return this;
    }
    ...
    convert(newNumerators, newDenominators, name) {
        return SassNumber.withUnits(this.convertValue(newNumerators, newDenominators, name), { numeratorUnits: newNumerators, denominatorUnits: newDenominators });
    }
    /**
    ...
    convertValue(newNumerators, newDenominators, name) {
        return this.convertOrCoerce({
            ...
            newNumeratorUnits: utils_1.asImmutableList(newNumerators),
            newDenominatorUnits: utils_1.asImmutableList(newDenominators),
            name,
        });
    }
    /**
    ...
    convertToMatch(other, name, otherName) {
        return SassNumber.withUnits(this.convertValueToMatch(other, name, otherName), {
    ...
    coerce(newNumerators, newDenominators, name) {
        return SassNumber.withUnits(this.coerceValue(newNumerators, newDenominators, name), { numeratorUnits: newNumerators, denominatorUnits: newDenominators });
    }
    /**
     * Returns `value`, converted to the units represented by `newNumerators` and
     * `newDenominators`.
    ...
    coerceToMatch(other, name, otherName) {
        return SassNumber.withUnits(this.coerceValueToMatch(other, name, otherName), {
            numeratorUnits: other.numeratorUnits,
            denominatorUnits: other.denominatorUnits,
        });
    }
    ...
    equals(other) {
        if (!(other instanceof SassNumber))
            return false;
        try {
            return utils_2.fuzzyEquals(this.value, other.convertValueToMatch(this));
        }
        catch (_a) {
            return false;
        }
    ...
    hashCode() {
        ...
        return (utils_2.fuzzyHashCode(canonicalValue) ^
            immutable_1.hash(canonicalNumerators) ^
            immutable_1.hash(canonicalDenominators));
    }
    toString() {
    ...
    convertOrCoerce(params) {
        ...
        const compatibilityError = () => {
            if ('other' in params) {
                ...
                return utils_1.valueError(message, params.name);
            }
            if (!otherHasUnits) {
                return utils_1.valueError(`Expected ${this} to have no units.`, params.name);
            }
            ...
            if (newNumerators.size === 1 && newDenominators.isEmpty) {
                ...
                if (type) {
                    return utils_1.valueError(`Expected ${this} to have a single ${type} unit (${unitsByType[type].join(', ')}).`, params.name);
                }
            }
            return utils_1.valueError(`Expected $this to have ${newNumerators.size + newDenominators.size > 1 ? 'units' : 'unit'} ${unitString(newNumerators, newDenominators)}.`, params.name);
        };
